...
oms::VideoInfo::VideoInfo() : thePrivateData(new oms::VideoInfoPrivateData),
                              pretty(true),
                              showsummary(true),
                              showdetails(true),
                              coredata(false),
...
bool oms::VideoInfo::updateTrackInfo(ossimRefPtr<ossimVMTITrack> track, ossimRefPtr<ossimPredatorKlvTable> klvTable, ossimRefPtr<ossimImageGeometry> geom, ossim_uint32 index) const
{
   ...
   ossimString polygon = "POLYGON((" + ossimString::toString(gul.lond()) + " " + ossimString::toString(gul.latd()) + "," + ossimString::toString(gur.lond()) + " " + ossimString::toString(gur.latd()) + "," + ossimString::toString(glr.lond()) + " " + ossimString::toString(glr.latd()) + "," + ossimString::toString(gll.lond()) + " " + ossimString::toString(gll.latd()) + "," + ossimString::toString(gul.lond()) + " " + ossimString::toString(gul.latd()) + " " + "))";
   ossimString point = "POINT(" + ossimString::toString(gcentroid.lond()) + " " + ossimString::toString(gcentroid.latd()) + ")";

   track->addToGroundTrack(polygon);
   track->addToCentroidTrack(point);
#if 0
  GEOSWKTReader* reader = GEOSWKTReader_create();
  GEOSGeometry* groundTrack = 0;
  try{
     groundTrack = GEOSWKTReader_read(polygon);
  }
  catch(...)
  {
  }
  GEOSWKTReader_destroy(reader);
  GEOSGeometry* composite = track->getGroundTrack();
  ...
  GEOSGeometry *centroidComposite = track->getCentroidTrack();
...
GEOSGeometry *geomFromSensorPosition(ossimRefPtr<ossimPredatorKlvTable> klvTable, ossim_float64 &lat, ossim_float64 &lon)
{
   ossim_float64 elev;
   ...
   if (klvTable->getSensorPosition(lat, lon, elev))
   {
      ...
      ossimString str(ossimString("POINT(") + ossimString::toString(lon) + 
                      ossimString(" ") + ossimString::toString(lat) + ossimString(")"));
...
GEOSGeometry *geomFromKlv(ossimRefPtr<ossimPredatorKlvTable> klvTable)
{
   ossimGpt wgs84;
   ossimGpt ul;
   ossimGpt ur;
...
//   std::cout << "*************************** " << distance/elevation.toDouble() << std::endl;
   ...
   result += indentation + "<groundGeom area=\"" + geoArea.string() + "\" elevation=\"" + elevation.string() + "\" klvnumber=\"" + klvnumber + "\" horizontalFOV=\"" + horizontalFOV.string() + "\" verticalFOV=\"" + verticalFOV.string() + "\" sensorDistance=\"" + sensorDistance.string() + "\" sensorPosition=\"" + sensorPosition.string() + "\" videoStartUTC=\"" + videostartutc.string() + "\" obliquityAngle=\"" + obliquityAngle.string() + "\" srs=\"epsg:4326\">" + groundGeometry.string() + "</groundGeom>" + separator;
}
...
std::string oms::VideoInfo::getInfo() const
{
   ...
   GEOSGeometry *sensorComposite = 0;
   //thePrivateData->thePredatorVideo->rewind();
   ...
   while (klvInfo.valid() && klvInfo->table())
   {
      ...
      if (needFileClassification && klvInfo->table()->getClassification(fileClassification))
         needFileClassification = false;
      if (needSensor && klvInfo->table()->getSensor(sensor))
         needSensor = false;
      if (needSensorFormat && klvInfo->table()->getSensorFormat(sensorFormat))
         needSensorFormat = false;
      if (needCountryCode && klvInfo->table()->getCountryCode(countryCode))
         needCountryCode = false;
      if (needCompartment && klvInfo->table()->getSecurityInformation(compartment))
         needCompartment = false;
      if (needDissemination && klvInfo->table()->getReleaseInstructions(dissemination))
         needDissemination = false;
      ...
      GEOSGeometry *geom = geomFromKlv(klvInfo->table());
      geomUnion(geom, &composite);
      if (!ossim::almostEqual(lastlat, lat) || !ossim::almostEqual(lastlon, lon))
      {
         GEOSGeometry *sensorGeom = geomFromSensorPosition(klvInfo->table(), lat, lon);
   ...
   if (composite)
   {
      ...
      unionGeom = GEOSWKTWriter_write(writer, composite);
   ...
   if (sensorComposite)
   {
      sensorUnionGeom = GEOSWKTWriter_write(writer, sensorComposite);
      GEOSGeom_destroy(sensorComposite);
